...
class RandomWindowBatchSampler(Sampler):
    ...
    def __init__(self, batch_size, window_size, seq_lens, drop_last=True, skip=0):
        valid_frames = []
        seq_len_cumsum = list(accumulate([0] + seq_lens))
        for j, len_j in enumerate(seq_len_cumsum[1:]):
            valid_frames += list(range(seq_len_cumsum[j], len_j - window_size + 1, skip + 1))
...
class SequentialWindowBatchSampler(Sampler):
    ...
    def __init__(self, batch_size, window_size, seq_lens, drop_last=True, skip=0):
        ...
        seq_len_cumsum = list(accumulate([0] + seq_lens))
        for j, len_j in enumerate(seq_len_cumsum[1:]):
            self.valid_frames += list(range(seq_len_cumsum[j], len_j - window_size + 1, skip + 1))
        self.batch_size = batch_size
        self.window_size = window_size
        self.drop_last = drop_last
