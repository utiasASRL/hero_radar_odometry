...
char const* run_steam_lm_sp(const p::object& meas_list, const p::object& match_list, const p::object& weight_list, 
    np::ndarray& poses, np::ndarray& vels, np::ndarray& lm_coords, const p::list& l_sp_list, bool use_gt,
    const np::ndarray& cov_diag) {
...
//  }
  ...
  for (unsigned int i = 0; i < window_size; i++) {
    ...
    if (use_gt && i > 0) {
      ...
      for (int c = 0; c < 6; ++c)
        cov(c,c) = double(p::extract<float>(cov_diag[c]));
      steam::L2LossFunc::Ptr unaryLossFunc(new steam::L2LossFunc());
      steam::BaseNoiseModel<6>::Ptr unaryNoiseModel(new steam::StaticNoiseModel<6>(cov));
      const TrajStateVar& state = states.at(i);
      lgmath::se3::Transformation pose_temp = state.pose->getValue();
      steam::TransformErrorEval::Ptr errorfunc(new steam::TransformErrorEval(pose_temp, T_k0_eval_ptr));
      steam::WeightedLeastSqCostTerm<6,6>::Ptr cost(
          new steam::WeightedLeastSqCostTerm<6,6>(errorfunc, unaryNoiseModel, unaryLossFunc));
      costTerms->add(cost);
    }
...
// (meas_list, match_list, pose_list, vel_list, lm_coords)
char const* run_steam_lm_spb(const p::object& meas_list, const p::object& match_list, const p::object& weight_list, 
    np::ndarray& poses, np::ndarray& vels, np::ndarray& lm_coords, const p::list& l_sp_list, bool use_gt,
    const np::ndarray& cov_diag) {
...
//  typedef steam::DoglegGaussNewtonSolver SolverType;
  ...
  keys.reserve(window_size - 1 + landmarks.size());
  for (unsigned int i = 1; i < states.size(); i++) {
    const TrajStateVar& state = states.at(i);
  ...
  for (unsigned int i = 0; i < landmarks.size(); i++) {
    const steam::se3::LandmarkStateVar::Ptr& landmark = landmarks.at(i);
    keys.push_back(landmark->getKey());
  }
  steam::BlockMatrix cov_blocks = solver.queryCovarianceBlock(keys);
  ...
  for (unsigned int i = 0; i < window_size; i++) {
    ...
    for (unsigned int j = 0; j < num_meas; ++j) {
      ...
      if (i > 0) {
        ...
        Eigen::Matrix<double, 9, 9> cov;
        int pose_block = i - 1;
        int lm_block = window_size - 1 + lm_id;
        cov.topLeftCorner(6,6) = cov_blocks.at(pose_block, pose_block);
        cov.bottomLeftCorner(3,6) = cov_blocks.at(lm_block, pose_block);
        cov.topRightCorner(6,3) = cov_blocks.at(lm_block, pose_block).transpose();
        cov.bottomRightCorner(3,3) = cov_blocks.at(lm_block, lm_block);
        Eigen::LLT<Eigen::MatrixXd> lltcov(cov);
        Eigen::MatrixXd L = lltcov.matrixL();
        ...
        // Eigen::MatrixXd cov = solver.queryCovariance(landmark->getKey());
        int lm_block = window_size - 1 + lm_id;
        Eigen::Matrix<double, 3, 3> cov = cov_blocks.at(lm_block, lm_block);
        Eigen::LLT<Eigen::MatrixXd> lltcov(cov);
        Eigen::MatrixXd L = lltcov.matrixL();
...
BOOST_PYTHON_MODULE(steampy_lm)
{
    ...
    p::def("run_steam_lm_spb", run_steam_lm_spb);
}
